...
**当前公开支持面**：`codex`（最完整的 governed 路径）· `claude-code` · `cursor` · `windsurf`（支持的安装与使用路径，且已接入 runtime adapter）· `openclaw`（preview runtime-core 路径）· `opencode`（preview adapter 路径）

...
**Use Case**: Already installed framework version, need to update to latest version
...
3. Map "Framework Only + Customizable Governance" to profile: `framework-only`
...
- However, if I modified officially managed paths, these changes may be overwritten
...
- Custom governance still exists
- Current state is still "framework only" and not misreported as workflow/full
...
- Whether current state is still governance framework foundation mode
- Whether dependency issues occurred
- Parts that still require manual handling

**Framework Version Special Note**:
What I currently have is the governance framework foundation, which does not mean the default workflow core is already complete.
